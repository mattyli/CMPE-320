...
// testing version of TestInsultGenerator.cpp
using namespace std;
...
int main() {
	...
	cout << ig.talkToMe() << endl;					// just needs to return a string
	...
	try {
		ig.generateAndSave("Nothingj.txt", 40000);
	} catch (NumInsultsOutOfBounds& e) {
		cerr << e.what() << endl;
	}
	cout << "\nSaving 1000 unique insults to a file...";
	try {
		ig.generateAndSave("SavedInsultsh.txt", 1000);
	} catch (FileException& e) {
		cerr << e.what() << endl;
		return 1;
	}
